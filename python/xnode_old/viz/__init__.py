...
import inspect
import os
from xnode_old.constants import VizModel, ExpansionMode

# TODO: potentially use these remnants of the old _get_view engine
...
# An enum which includes every color a View can take.
...
class Viz:
    """Interface for new View objects.

    Each Primitive and Layout should be a subclass of ``View``. The subclasses should also implement `compile_full()`,
    ...
    def __init__(self, summary: Optional[str],
        ...
        Args:
            summary: An optional name to assign to this View, which will be shown in its summary expansion mode.
            expansion_mode: The expansion state this View should take if none is specifically given in the watch
                expression.
        """
        self._summary: Optional[str] = summary
        self.default_expansion_mode: Optional[ExpansionMode] = expansion_mode

    def compile_full(self) -> Tuple[VizModel, Iterable['View']]:
        """Creates a VizModel which describes this View and can be sent to a client for rendering.

        Returns:
            A VizModel describing the properties of this View in its entirety.
            A list of all View objects referenced in the VizModel.
        """
        raise NotImplementedError

    def compile_compact(self) -> Tuple[VizModel, Iterable['View']]:
        """Creates a compact VizModel which describes this View and can be sent to a client for rendering.

        Returns:
            A VizModel describing a glimpse of the properties of this View.
            A list of all View objects referenced in the VizModel.
        """
        raise NotImplementedError

    def compile_summary(self) -> 'TextPrimitiveModel':
        """Creates a limited-size VizModel giving a hint as to the View's properties.

        Returns:
            A TextPrimitiveModel whose text is the name of this View if given in the constructor, or otherwise this
    ...
    def __str__(self) -> str:
        """Returns a string which describes the basic properties of the View.

        For example, "list[5]" or "tensor[float]".

        Returns:
            A brief string representation of the View.
        """
        raise NotImplementedError
...
class Grid(Viz):
   ...
   """
    ...
    # How many key-value pairs to show in the compact form of this View.
    COMPACT_COLS = 5
    COMPACT_ROWS = 5
...
class GridLayoutModel(VizModel):
    def __init__(self, elements: List[Tuple['View', int, int, int, int]]):
...
class FlowLayoutModel(VizModel):
    def __init__(self, elements: List['View']):

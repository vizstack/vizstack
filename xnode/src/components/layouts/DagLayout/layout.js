...
export type DagNodeLayoutedSpec = {
    ...
    // Can be specified by client (for childless nodes), but might be overwritten by layout.js
    width: ?number,
    height: ?number,

    // Will be determined by layout.js
    x: ?number,
    y: ?number,
};
...
class GraphProperties {
    constructor(nodes: Array<DagNodeLayoutedSpec>, edges: Array<DagEdgeLayoutedSpec>) {
        this._nodes = {};
    ...
    _setHierarchyHeight(nodeId: DagElementId) {
        ...
        this._nodeHierarchyHeights[nodeId] = Math.max(children.map(childId => this._nodeHierarchyHeights[childId])) + 1;
    }
 ...
 */
export function getElkGraph(nodes: Array<DagNodeLayoutedSpec>, edges: Array<DagEdgeLayoutedSpec>) {
...
class ELKEdgeSegment {
     ...
     * @param graph
     */
    constructor(edgeId: DagEdgeId, startNodeId: DagElementId, startPortId: string, endNodeId: DagElementId, endPortId: string, graph: GraphProperties) {
        ...
        while (startNodeId !== endNodeId) {
            if (
                startNodeId === null ||
                (endNodeId !== null && graph.getHierarchyHeight(startNodeId) > graph.getHierarchyHeight(endNodeId))
            ) {
        ...
        while (true) {
            ...
            fromEdgeSegments.push(
                new ELKEdgeSegment(
                    ...
                    getPortId(graph.getParent(startId), outPortCounts[graph.getParent(startId)], false),
                ),
            );
 ...
 */
function createElkNode(node: DagNodeLayoutedSpec, numOutPorts: number, numInPorts: number, graph: GraphProperties) {
    const elkNode = {
        id: node.id,
        properties: {

...
from xn.viz import VIZ_FN, get_viz


class Nestable:
    """A parent class to `FunctionCall` and `GraphContainer`, representing an object which might be nested in a hierarchy of
    containers."""
    def __init__(self):
...
# Graph operations.
# -----------------
# A `FunctionCall` object records a single function call and is created whenever a function wrapped by an `TrackedFunction`
# object is called.
# ======================================================================================================================
...
class GraphOp(Nestable):
    ...
    def __init__(self, fn, fn_name, args, kwargs):
        ...
        # Arguments which were tracked via a call to `_track_data()` (that is, should be shown in the graph) have a
        ...
        # A `FunctionCall` object always has temporal level 0, so any new temporal container will be able to encapsulate
...
class GraphData:
    ...
    def __init__(self, obj, creator_op=None, creator_pos=-1):
        ...
        `FunctionCall` or tracked via `_track_data()`. `obj` should be immutable; if it changes, then the graph will be
        rendered incorrectly.

        Args:
            obj (object): The immutable object wrapped by the `GraphData`.
            creator_op (GraphOp): The `FunctionCall` which created the wrapped object.
...
# Graph containers.
# -----------------
# A `GraphContainer` groups together `FunctionCall` and other `GraphContainer` objects to provide abstraction in
# visualization. Containers are either abstractive, which collapses a subnetwork into a "black box" function,
# or temporal, which conceals everything outside of it, showing only the single "flow" it contains. See get_viz.md for
...
class GraphContainer(Nestable):
    """Represents a collection of grouped `FunctionCall` and `GraphContainer` objects."""
    def __init__(self, contents, name='null', temporal_level=0, temporal_step=-1):
        """Constructor.
        Args:
            contents (set): A list of `FunctionCall` and `GraphContainer` objects grouped by the instance,
...
def _build_abstractive_container(outputs, inputs, container_name):
    ...
    `GraphData` which is in the set of inputs. Whenever a new `FunctionCall` is found during search, its outermost parent
...
def track_data(obj, creator_op=None, creator_pos=-1):
    ...
    Args:
        ...
        creator_op (GraphOp or None): The `FunctionCall` object recording the function call which outputted `obj`. If
    ...
    if not obj.__class__.__name__.startswith('__XNODE_GENERATED__'):
        ...
        try:
            obj.__class__ = type('__XNODE_GENERATED__{}'.format(obj.__class__.__name__), (obj.__class__,), new_class_dict)
        except TypeError:
...
def has_graphdata(obj):
    ...
    Objects which are tracked in the computation graph via a call to `_track_data()` have `GraphData` objects associated
    ...
    def __init__(self, obj, output_props_to_surface=None, op_name=None):
        ...
        Args:
            ...
            output_props_to_surface (list or None): A dict determining what properties of each `GraphData`
                ...
                c = TrackedFunction(f, output_props_to_surface=[{'Arbitrary Value': 'prop1'}, {'Another Value': 'prop2'}])
            ...
            op_name (str or None): The name that should be given to created `FunctionCall` objects. If `None`,
...
class AbstractContainerGenerator(wrapt.ObjectProxy):
    """Wraps a callable object, causing all `FunctionCall` and `GraphContainer` objects created during its execution to be
    contained in a new abstractive `GraphContainer`."""
    def __call__(self, *args, **kwargs):
        """Executes the wrapped function and nests every `FunctionCall` and `GraphContainer` it creates inside a new
        `GraphContainer`.

        `AbstractContainerGenerator` expects that its wrapped function contains (perhaps further nested within other
        functions) some `TrackedFunction` functions that will produce `FunctionCall` objects when called. Further, it expects
        that at least some of the inputs and outputs of the wrapped function have `GraphData` objects.
        These objects are then used to identify every `FunctionCall` that exists between the inputs and the outputs in the
